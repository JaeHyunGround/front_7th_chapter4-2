...
// 캐싱된 fetcher 생성
const cachedFetchMajors = createCachedFetcher(() =>
  axios.get<Lecture[]>("/schedules-majors.json")
);
const cachedFetchLiberalArts = createCachedFetcher(() =>
  axios.get<Lecture[]>("/schedules-liberal-arts.json")
);

// 개선된 API 호출: 병렬 실행 + 캐싱
const fetchAllLectures = async () => {
  // Promise.all로 병렬 실행 (await를 제거하여 병렬 실행)
  // 캐싱 덕분에 실제 네트워크 요청은 2번만 발생
...
const SearchDialog = ({ searchInfo, onClose }: Props) => {
  ...
  // 전공 목록 메모이제이션 (lectures가 변경될 때만 재계산)
  ...
  // 강의 추가 함수 메모이제이션 (의존성이 변경될 때만 함수 재생성)
